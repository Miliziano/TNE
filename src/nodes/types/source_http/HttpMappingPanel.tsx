...
const FIXED_FIELDS: OutputField[] = [
  { id: 'f_status_code',  name: 'status_code',  type: 'integer', fixed: true, hint: 'HTTP response code' },
  { id: 'f_content_type', name: 'content_type', type: 'string',  fixed: true, hint: 'Content-Type header'  },
  { id: 'f_latency_ms',   name: 'latency_ms',   type: 'integer', fixed: true, hint: 'Response time in ms' },
  { id: 'f_headers',      name: 'headers',      type: 'object',  fixed: true, hint: 'Response headers'     },
]
...
const RESPONSE_TYPE_FIELDS: Record<string, OutputField[]> = {
  ...
  json_raw: [
    { id: 'f_body',        name: 'body',        type: 'string', fixed: true, hint: 'Raw JSON body as string'   },
    { id: 'f_body_parsed', name: 'body_parsed', type: 'object', fixed: true, hint: 'JSON body already parsed as object' },
  ],
  text: [
    { id: 'f_body', name: 'body', type: 'string', fixed: true, hint: 'Raw text body' },
  ],
  xml: [
    { id: 'f_body', name: 'body', type: 'string', fixed: true, hint: 'Raw XML body' },
  ],
  csv: [
    { id: 'f_body', name: 'body', type: 'string', fixed: true, hint: 'Raw CSV body' },
  ],
  binary: [
    { id: 'f_content',        name: 'content',        type: 'string',  fixed: true, hint: 'Body in base64'       },
    { id: 'f_content_length', name: 'content_length', type: 'integer', fixed: true, hint: 'Size in bytes'   },
  ],
  pdf: [
...
const RESPONSE_TYPE_LABELS: Record<string, string> = {
  json:     'JSON — extracted fields',
  json_raw: 'JSON raw — full body',
  text:     'Text',
  xml:      'XML',
  csv:      'CSV',
  binary:   'Binary',
...
export function HttpMappingPanel({ nodeId }: { nodeId: string }) {
  ...
  const addField = useCallback(() => {
    ...
    saveAndPropagate([...customFields, {
      id: `cf_${n}`, name: `field_${n}`, type: 'string', fixed: false,
    }])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ padding: '8px 12px', background: '#1a2030', borderRadius: 6, border: '0.5px solid #2a3349', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div>
          <div style={{ fontSize: 11, color: '#c8d4f0', fontWeight: 600 }}>
            Response type: <span style={{ color: '#4a9eff' }}>{RESPONSE_TYPE_LABELS[responseType] ?? responseType}</span>
          </div>
          <div style={{ fontSize: 10, color: '#8593b5' }}>
            Configure the type in the Configuration tab · the fixed fields change accordingly
          </div>
        </div>
      </div>
      ...
      <div style={{ background: '#161b27', border: '1px solid #2a3349', borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ padding: '8px 12px', background: '#1a2030', borderBottom: '1px solid #2a3349', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: '#9a9aaa' }}>Fixed fields — always present</span>
        </div>
      ...
      {responseType === 'json_raw' && (
        <div style={{ padding: '8px 12px', background: '#1a1030', border: '1px solid #3a1a6a', borderRadius: 6, display: 'flex', alignItems: 'flex-start', gap: 8 }}>
          ...
          <div style={{ fontSize: 10, color: '#a78bfa' }}>
            In <strong>JSON raw</strong> mode the full body is passed to downstream nodes without extracting fields.
            Use a <strong>JSON Parser</strong> or <strong>Script</strong> node after this to process <code>body_parsed</code>.
          </div>
        </div>
      )}
      ...
      {showCustomFields && (
        <div style={{ background: '#161b27', border: '1px solid #2a3349', borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ padding: '8px 12px', background: '#1a2030', borderBottom: '1px solid #2a3349', display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#c8d4f0' }}>JSON fields to extract</div>
              <div style={{ fontSize: 9, color: '#8593b5' }}>
                Declare the JSON fields you want to propagate to downstream nodes
              </div>
            </div>
          ...
          {customFields.length === 0 ? (
            <div style={{ padding: '16px 12px', fontSize: 10, color: '#2a3349', fontStyle: 'italic', textAlign: 'center' }}>
              Add the fields that the response JSON contains — or use the "Test connection" button to detect them automatically
            </div>
          ) : (
            <div style={{ padding: '4px 0' }}>
      ...
      <div style={{ padding: '6px 10px', fontSize: 10, color: '#8593b5', fontStyle: 'italic', background: '#1a2030', borderRadius: 4, border: '0.5px solid #2a3349', display: 'flex', alignItems: 'flex-start', gap: 5 }}>
        ...
        {responseType === 'json' && ' Use "Test connection" in the Configuration tab to detect the fields automatically.'}
        {responseType === 'json_raw' && ' Use a Script or JSON Parser node to process the full body.'}
      </div>
