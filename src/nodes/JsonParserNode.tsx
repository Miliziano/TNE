...
export const JsonParserNode = memo(({ id, data, selected }: NodeProps) => {
  const nodeData   = data as NodeData
  const selectNode = useFlowStore((s) => s.selectNode)
  const [showModal, setShowModal] = useState(false)

  const config    = nodeData.config?.jsonParser as JsonParserConfig | undefined
  ...
  const displayName = nodeData.config?.displayName || 'JSON Parser'
  ...
  return (
    <div
      ...
      title="JSON Parser — doppio click per configurare"
      style={{
        minWidth:     150,
        minHeight,
        borderRadius: 8,
        ...
        background:   '#1e2535',
        boxShadow:    selected ? `0 0 0 2px rgba(34,211,238,0.35)` : undefined,
        userSelect:   'none',
        cursor:       'pointer',
        position:     'relative',
        display:      'flex',
    ...
    >
      ...
      <div
        ...
        title="Apri configuratore JSON Parser"
        style={{
          position:       'absolute',
          top:            -8,
          right:          -8,
      ...
      <div style={{ padding: '5px 8px', flex: 1, display: 'flex', flexDirection: 'column', gap: 3 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <i className="ti ti-braces" style={{ fontSize: 9, color: ACCENT, flexShrink: 0 }} />
      ...
      {flows.map((flow, idx) => {
        ...
        return (
          <Handle
            ...
            title={`${flow.label} — ${flow.jsonPath}`}
          />
        )
      })}
