        ...
        for (node_id_str, stats) in stats_map.iter() {
            ...
            if verso_valle_morta {
                push_event(crate::engine::events::EngineEvent::NodeLog {
                    ...
                    row_num:    0,
                    // Il nodo va NEL TESTO: il pannello non antepone il
...
async fn run_node(
    ...
    match node_type.as_str() {
        ...
        "source_mqtt" => {
            let rx = take_single_input(&mut inputs);
            let tx = take_primary_output(&mut outputs);
            super::nodes::source_mqtt::run(ctx, rx, tx).await
        }

        "webhook_receiver" => {
            let rx = take_single_input(&mut inputs);
            let tx = take_primary_output(&mut outputs);
            super::nodes::webhook_receiver::run(ctx, rx, tx).await
        }
        ...
        t if is_stub(t) => {
            ...
            let stats = NodeStats {
                ...
                error: Some(format!(
                    "Node '{}' is not yet implemented in the engine: the rows are forwarded without being processed. The result is NOT reliable.",
                    node_type
                )),
            };
