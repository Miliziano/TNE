...
import type { Row, RunnerCallbacks, ExecutionContext, AnyExecutor } from '../io/types'
import { isStreamingExecutor, deserializeLaneValue } from '../io/types'
...
async function executeStreamingNode(
  ...
  outputs:  Map<string, Map<string, Row[]>>,
  context:  ExecutionContext,
  executor: import('../io/types').StreamingNodeExecutor,
): Promise<boolean> {
  const input    = collectInput(node.id, edges, outputs)
  ...
  const onRow = async (row: Row): Promise<void> => {
    ...
    for (const downstream of sortedDownstream) {
      ...
      if (downExec && !isStreamingExecutor(downExec)) {
        const requiresComplete = (downExec as import('../io/types').NodeExecutor).requiresCompleteInput
        if (requiresComplete) {
      ...
      try {
        const downOutput = await (downExec as import('../io/types').NodeExecutor).execute(downstream, downInput, context)
        const durationMs = Date.now() - startMs
