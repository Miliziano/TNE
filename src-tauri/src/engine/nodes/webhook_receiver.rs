...
pub async fn run(
    ...
    // Config — chiavi allineate a resolveReceiverConfig del runner.
    let resource_id = {
        let r = spec.str_or("resourceId", "");
        if r.is_empty() { format!("webhook_{}", ctx.node_id.0) } else { r }
    };
    let port = spec.u64_or("port", 8088) as u16;
    let path = {
        let p = spec.str_or("path", "/webhook");
        if p.starts_with('/') { p } else { format!("/{}", p) }
    };
    let secret      = spec.str_or("secret", "");
    let sig_header  = spec.str_or("sigHeader", "x-hub-signature-256");
    let sig_algo    = spec.str_or("sigAlgo", "sha256");
    let dedup_ttl   = spec.u64_or("dedupTtlSec", 0);
    let max_buffer  = spec.u64_or("maxBuffer", 1000) as usize;
    let overflow    = spec.str_or("overflow", "drop_oldest");
    // ip_whitelist: prop stringa separata da virgole (vuota = nessun filtro).
    let ip_whitelist: Vec<String> = spec.str_or("ipWhitelist", "")
        .split(',')
