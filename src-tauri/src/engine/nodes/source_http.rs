...
pub(crate) async fn auth_headers(spec: &Spec)
    ...
    match spec.str_or("authType", "none").as_str() {
        ...
        "digest" => {
            return Err("auth 'digest' non ancora in questa build — arriva nel patch successivo \
                        (richiede il crate md-5 e lo scambio challenge-response)".into());
        }
        other => {
...
pub(crate) async fn execute_single_request(
    ...
    Err(format!("HTTP fallita dopo {} tentativi: {}", retry_count + 1, last_err))
}
